...
const publicProposalPostService = require("../../services/democracy.service")("democracyPublicProposal");
...
async function updatePost(ctx) {
  ...
  ctx.body = await publicProposalPostService.updatePost(
    chain,
    postId,
    title,
    content,
...
async function getPosts(ctx) {
  ...
  ctx.body = await publicProposalPostService.getPostsByChain(chain, page, pageSize);
}

async function getPostById(ctx) {
  const { chain, postId } = ctx.params;

  ctx.body = await publicProposalPostService.getPostById(chain, postId);
}
