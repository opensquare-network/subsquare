...
const postService = require("../../services/post.service")("motion");
const motionPostService = require("../../services/motion.service");
...
async function getPosts(ctx) {
  ...
  ctx.body = await motionPostService.getPostsByChain(chain, page, pageSize);
}

async function getPostById(ctx) {
  const { chain, postId } = ctx.params;

  ctx.body = await motionPostService.getPostById(chain, postId);
}
