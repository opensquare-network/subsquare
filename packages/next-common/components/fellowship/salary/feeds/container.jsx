...
import FilterButton from "next-common/lib/button/filterButton";
    ...
    <>
        <FilterContainer className="max-md:flex-col max-md:gap-2 md:flex-row md:items-center md:justify-between mb-4 pr-6">
            ...
            <div className='flex max-md:flex-col md:flex-row md:items-center max-md:w-full gap-2'>
                {component}
                <SearchBox value={searchValue} setValue={setSearchValue} placeholder={'Search address'}/>
                <FilterButton className={'max-md:w-full'} loading={loading} size="small" onClick={onFilter}>Filter</FilterButton>
            </div>
        </FilterContainer>
