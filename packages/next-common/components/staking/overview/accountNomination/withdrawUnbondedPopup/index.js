...
function WithdrawUnbondedPopupContent() {
  const api = useContextApi();
  const { decimals, symbol } = useChainSettings();
  const { unlocked, loading: isBalanceLoading } = useStakingBalance();

  const getTxFunc = useCallback(async () => {
    if (!api) {
      return null;
    }
    return api.tx.staking.withdrawUnbonded(0);
  }, [api]);
...
export default function WithdrawUnbondedPopup({ onClose }) {
  return (
    <SignerPopupWrapper onClose={onClose}>
      <Popup title="Withdraw Unbonded" onClose={onClose}>
        <WithdrawUnbondedPopupContent />
      </Popup>
    </SignerPopupWrapper>
  );
}
