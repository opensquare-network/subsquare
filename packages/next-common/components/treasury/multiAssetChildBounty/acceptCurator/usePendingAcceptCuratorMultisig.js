...
// Detect whether an accept curator multisig for this child bounty is already
// in progress on-chain, initiated by ANY group that can act as the child
// curator - not just the multisig routes of the connected user. e.g. another
// group may have started the operation through a delegate multisig the user is
// not a member of; the user should still be warned before starting their own
...
export default function usePendingAcceptCuratorMultisig(
  ...
  useEffect(() => {
    ...
    if (!api || isNil(parentBountyId) || isNil(childBountyId) || !curator) {
      return;
    }

    // accept_curator(parent_bounty_id, child_bounty_id). Must match what the
    // popup submits.
  ...
  }, [api, parentBountyId, childBountyId, curator, authority]);
