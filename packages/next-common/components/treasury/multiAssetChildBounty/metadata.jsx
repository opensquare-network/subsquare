...
function MetaGuard({ children }) {
  ...
  const shouldShow = useMemo(() => {
    return ["Awarded", "Paid"].includes(state);
  }, [state]);
    ...
    <SecondaryCardDetail>
      ...
      <Info>
        ...
        <SideInfoItem>
          ...
          <SideInfoItemValue>
            <ValueDisplay
              ...
              value={toPrecision(onChain.value ?? 0, decimals)}
              symbol={symbol}
            />
          </SideInfoItemValue>
        </SideInfoItem>
        <SideInfoItem>
          ...
          <SideInfoItemValue>
            <Anchor
              href={`/treasury/multi-asset-bounties/${onChain.parentBountyId}`}
            >
...
export default function MultiAssetChildBountyMeta() {
  return (
    <MetaGuard>
      <Meta />
    </MetaGuard>
