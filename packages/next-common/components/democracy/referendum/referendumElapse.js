...
export default function ReferendumElapse({ detail }) {
  ...
  const isFinished = (timeline || []).some((item) => referendumVoteFinishedStatusArray.includes(item.method));
  const startHeight = onchain.indexer?.blockHeight;
  const meta = getMeta(onchain);
  const estimatedBlocksTime = useEstimateBlocksTime(
    meta.end - blockHeight,
  );
  ...
  return <CountDown
      ...
      tooltipContent={`End in ${estimatedBlocksTime}, #${bigNumber2Locale(meta.end.toString())}`}
    />;
}
