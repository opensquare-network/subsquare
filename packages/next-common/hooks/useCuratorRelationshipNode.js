...
export default function useCuratorRelationshipNode() {
  ...
  const multisigAddress = useMultisigAddress(curator);
  const signatoryMultisig = useSignatoryMultisig(curator);
  const identityInfo = useFetchIdentityInfo(curator);
  ...
  const rootNode = createRootNode(curator, multisigAddress);
