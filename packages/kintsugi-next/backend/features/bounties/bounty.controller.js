...
const postService = require("../../services/post.service")("bounty");
...
async function updatePost(ctx) {
  ...
  ctx.body = await bountyPostService.updatePost(
    postId,
    title,
    content,
    contentType,
...
async function getPosts(ctx) {
  ...
  ctx.body = await bountyPostService.getPostsByChain(page, pageSize);
}

async function getPostById(ctx) {
  const { postId } = ctx.params;

  ctx.body = await bountyPostService.getPostById(postId);
}
...
async function setPostReaction(ctx) {
  ...
  const user = ctx.user;
  ctx.body = await postService.setPostReaction(postId, reaction, user);
}
