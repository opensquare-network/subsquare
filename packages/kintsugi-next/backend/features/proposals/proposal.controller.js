...
async function updatePost(ctx) {
  ...
  ctx.body = await treasuryProposalPostService.updatePost(
    postId,
    title,
    content,
    contentType,
...
async function getPosts(ctx) {
  ...
  ctx.body = await treasuryProposalPostService.getPostsByChain(page, pageSize);
}

async function getPostById(ctx) {
  const { postId } = ctx.params;

  ctx.body = await treasuryProposalPostService.getPostById(postId);
}
